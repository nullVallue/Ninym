...
folder_mapping_list = {
    "pretrained_v2/": "models/pretraineds/hifi-gan/",
    "refinegan/": "models/pretraineds/refinegan/",
    "embedders/contentvec/": "models/embedders/contentvec/",
    "predictors/": "models/predictors/",
    "formant/": "models/formant/",
}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Download RVC prerequisites")
    parser.add_argument(
        "--hifigan", action="store_true", help="Download HiFi-GAN pretraineds"
    )
    parser.add_argument("--models", action="store_true", help="Download predictor models")
    parser.add_argument("--exe", action="store_true", help="Download executables (ffmpeg)")
    parser.add_argument("--all", action="store_true", help="Download all prerequisites")
    ...
    if not any([args.hifigan, args.models, args.exe]):
        print("No options selected. Downloading all by default...")
        args.hifigan = True
        args.models = True
        args.exe = True

    prequisites_download_pipeline(args.hifigan, args.models, args.exe)
